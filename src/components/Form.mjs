	...
	return (
		<>
			...
			<div className='overflow-auto space-x-8 mx-4 flex  justify-between'>
				...
				<button className="p-2 bg-yellow-500 rounded text-white" onClick={() => { document.querySelector("body").classList.toggle("bg-black"); document.querySelector("#nav").classList.toggle("bg-gray-500");document.querySelector("#preview").classList.toggle("text-white");document.querySelector("#status").classList.toggle("text-white");if (dark_text === "Enable Dark Mode") { set_dark_text("Disable Dark Mode") } else { set_dark_text("Enable Dark Mode") } }}>	{dark_text}	</button>
